...
------renaming column name for id-----
sp_rename 'credit_card_transcations.index','transaction_id'
...
with cte as (
select *,sum(amount) over(partition by card_type order by date,transaction_id) as total_spend
from credit_card_transcations
--order by card_type,total_spend desc
)
